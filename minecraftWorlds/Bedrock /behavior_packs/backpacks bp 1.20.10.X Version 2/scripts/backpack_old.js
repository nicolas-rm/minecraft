		...
		system.runInterval(function tick() {			
			if(system.currentTick%40 == 0){
				...
				for(let bpTempEntity of bpTempEntities){
					let nearbyPlayerQuery = {type:"minecraft:player",location:bpTempEntity.location,dimension:bpTempEntity.dimension,maxDistance:3}
					let nearbyPlayers = world.getDimension('overworld').getEntities(nearbyPlayerQuery);
