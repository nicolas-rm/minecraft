...
export default class backpack {
	static main(){           
		...
		*/
		...
		/*
		*	INITIALIZE BP CENTRAL
		*/
		let setup = system.runInterval(()=>{			
			for(let player of world.getPlayers()){
				let mainCentralQuery = {type:'bps:bp_central'}
                 ...
                 if(centralEntity.length==0){
					...
					system.clearRun(setup);
					system.runInterval(backpackTick,1)
                }else{
		...
		function backpackTick() {			
			...
			for(let player of players){					
				...
				let getBackpack = playerInv.getItem(player.selectedSlot);				
		...
		function avoidDuplicateID(id){
			...
			let eQuery = {type:`bps:container_entity_new`,tags:[id]}
			...
			if(bpsTag.length<=0){
				...
				if(backpackItemLore.length>0){
					...
					setupBackpackTempInventory(backPackUID,player.dimension,player.location,bpSize);
						...
						player.sendMessage(`[Backpack+] §c${e}.`);
						return;
					}
				}else{			
				...
				if(bpEntityTemp.length>0){
					bpEntityTemp[0].teleport(playerLocation)
					bpEntityTemp[0].triggerEvent("timeout")
				}
				if(block){
					if(block.typeId.includes("portal")){
						if(bpEntityTemp.length>0){
							bpEntityTemp[0].triggerEvent("despawn2")
						}
			...
			switch(command){
					...
					if(eventData.sender.getTags().filter(tag=>tag == 'op').length>0){
						...
						if(bp.length>0) {
							...
							for(let slot=0; bpInv.size>slot; slot++){
								...
								if(color == 0){
									color=1
									itemList = itemList + ` §2${newName}` + " x" + `${item.amount}` + "§e,";
								}else{
									color=0
									itemList = itemList + ` §a${newName}` + " x" + `${item.amount}` + "§e,";
								}									
								
							}
							eventData.sender.sendMessage(`[Backpack+] ${itemList}`);
						}else{
				...
				case "bp list":
					...
					if(eventData.sender.getTags().filter(tag=>tag == 'op').length>0){
						...
						eventData.sender.sendMessage(`[Backpack+] ${bpList}`);
					}else{
						eventData.sender.sendMessage(`[Backpack+] §cMust be an OP to use this command.`);
					}
				break;
